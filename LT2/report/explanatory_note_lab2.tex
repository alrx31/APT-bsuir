...
\textbf{Скриншоты графических представлений первого, второго и третьего Activity}

\textbf{Первое Activity — RegistrationActivity}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{screenshots/auth.jpg}
\caption{Activity (RegistrationActivity)}
\label{fig:registration}
\end{figure}


\textbf{Второе Activity — TaxiActivity}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{screenshots/main.jpg}
\caption{Activity (TaxiActivity)}
\label{fig:taxi}
\end{figure}


\textbf{Третье Activity — RouteActivity}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{screenshots/path.jpg}
\caption{ Activity (RouteActivity)}
\label{fig:route}
\end{figure}


\textbf{Код XML-файлов графических представлений первого, второго и третьего Activity}

\textbf{XML-файл первого Activity — activity\_registration.xml}
...
\textbf{XML-файл второго Activity — activity\_taxi.xml}
...
\textbf{XML-файл третьего Activity — activity\_route.xml}
...
\textbf{Код Kotlin-файлов первого, второго и третьего Activity}

\textbf{Kotlin-файл первого Activity — RegistrationActivity.kt}
...
использование \texttt{SharedPreferences} для сохранения и восстановления данных пользователя между сеансами; проверка наличия сохраненных данных для определения режима работы (регистрация или вход); явный вызов второго Activity (\texttt{TaxiActivity}) с передачей данных через \texttt{Intent} и методы \texttt{putExtra()}; логирование всех методов жизненного цикла Activity с использованием класса \texttt{Log} и тега \texttt{TAG}.

\textbf{Kotlin-файл второго Activity — TaxiActivity.kt}
...
получение данных из \texttt{Intent} с помощью методов \texttt{getStringExtra()}; неявный вызов третьего Activity через \texttt{startActivityForResult()} с использованием action и category; обработка результата от третьего Activity в методе \texttt{onActivityResult()}; сохранение и восстановление маршрута с помощью \texttt{SharedPreferences}; использование класса \texttt{Toast} для отображения всплывающих сообщений; логирование всех методов жизненного цикла Activity.

\textbf{Kotlin-файл третьего Activity — RouteActivity.kt}
...
явный вызов \texttt{MapActivity} через \texttt{startActivityForResult()} для выбора точки на карте; обработка результата от \texttt{MapActivity} в методе \texttt{onActivityResult()}; возврат данных во второе Activity через \texttt{setResult()} и \texttt{finish()}; использование класса \texttt{Toast} для отображения информационных сообщений; логирование всех методов жизненного цикла Activity.
...
создание нескольких Activity и управление их жизненным циклом; явный и неявный вызов Activity с помощью класса \texttt{Intent}; передача данных между Activity через \texttt{Intent} и методы \texttt{putExtra()}; получение результата от Activity через \texttt{startActivityForResult()} и \texttt{onActivityResult()}; хранение данных с помощью класса \texttt{SharedPreferences}; создание всплывающих сообщений с помощью класса \texttt{Toast}; логирование работы приложения с помощью класса \texttt{Log} и анализ логов в LogCat.
...
Всплывающее сообщение создают через Toast: формируют объект вызовом Toast.makeText, передавая контекст (например, текущую Activity), текст и длительность показа, после чего вызывают show(). Длительность задаётся константами LENGTH\_SHORT (короткий показ) или LENGTH\_LONG (более длительный показ).

\textbf{2. В каких случаях необходимо логирование?}

Логирование используют, когда нужно понять, как именно работает приложение: при отладке и поиске ошибок, при анализе падений и обращений пользователей, для наблюдения за жизненным циклом компонентов, а также при измерении производительности и выявлении «узких мест». Кроме того, логи помогают фиксировать важные события и состояния, чтобы позже восстановить картину происходившего.

\textbf{3. Что представляет собой окно LogCat? Какие существуют уровни логирования?}

LogCat — это окно в Android Studio, в котором отображаются сообщения, которые пишут само приложение и система Android. Оно удобно тем, что позволяет искать и фильтровать записи по уровню важности, тегу и другим параметрам.

Основные уровни логирования (от менее критичных к более критичным): VERBOSE (подробные технические сообщения), DEBUG (информация для отладки), INFO (обычные информационные сообщения), WARN (предупреждения), ERROR (ошибки) и ASSERT/WTF (критические ситуации, которые не должны происходить в нормальном режиме).

\textbf{4. Как программно реализовать логирование?}

Логирование реализуют через класс Log, выбирая метод в зависимости от уровня важности сообщения (verbose, debug, info, warn, error и т.д.). Обычно передают TAG (идентификатор, чаще всего имя класса) и текст сообщения, чтобы затем удобно фильтровать записи в LogCat. При обработке ошибок дополнительно можно передать исключение, чтобы в логе был виден стек вызовов.

\textbf{5. Как создать новое Activity (опишите работу с java-классом, layout-файлом, файлом конфигурации AndroidManifest.xml)?}

Новое Activity создают так: сначала добавляют класс (на Java или Kotlin), который наследуется от Activity/AppCompatActivity, и в onCreate задают разметку через setContentView. Затем создают XML-разметку в каталоге res/layout и описывают в ней интерфейс. После этого Activity регистрируют в AndroidManifest.xml внутри секции application, указывая android:name с именем класса; если планируется неявный запуск, дополнительно добавляют intent-filter с нужным action и category.

\textbf{6. Для чего используется контекст приложения Context?}

Context нужен как «точка доступа» к окружению приложения: через него получают ресурсы (строки, цвета и т.п.), обращаются к системным сервисам, открывают SharedPreferences, работают с файлами и БД, а также запускают компоненты (Activity, Service, BroadcastReceiver). Activity сама является Context, поэтому часто в качестве контекста используют this.

\textbf{7. Для чего используются объекты класса Intent?}

Intent применяют как описание «намерения» выполнить действие: запустить конкретный экран (явно) или запросить выполнение действия по описанию (неявно), передать параметры между компонентами, стартовать сервис или отправить широковещательное сообщение. Также Intent используют, когда нужно запустить Activity и затем получить от него результат.

\textbf{8. Как выполнить явный вызов Activity?}

Явный запуск выполняют, когда точно известно, какой экран нужно открыть: создают Intent, указывают текущий контекст и класс целевого Activity, при необходимости добавляют данные (extras), затем вызывают startActivity.

\textbf{9. Как выполнить неявный вызов Activity?}

Неявный запуск делают через описание действия: в Intent задают action, при необходимости добавляют category и другие параметры (например, data). Дальше система выбирает подходящее Activity по intent-filter, поэтому у целевого Activity в AndroidManifest.xml должен быть объявлен фильтр с тем же action и подходящей категорией (обычно DEFAULT).

\textbf{10. Какие состояния предусмотрены жизненным циклом Activity?}

В жизненном цикле Activity выделяют состояния: created (создано), started (видимо), resumed (в фокусе и доступно для взаимодействия), paused (теряет фокус), stopped (не видно, но ещё существует) и destroyed (уничтожено).

\textbf{11. Какие методы автоматически срабатывают при смене состояния Activity? Как можно использовать эти методы?}

При смене состояния Android вызывает методы жизненного цикла: onCreate, onStart, onResume, onPause, onStop, onRestart и onDestroy. Их используют, чтобы правильно инициализировать интерфейс, запускать и останавливать ресурсоёмкие операции (камера, GPS, сетевые подписки), сохранять состояние и данные, освобождать ресурсы, а также писать диагностические логи.

\textbf{12. Как выполняется передача данных с помощью Intent?}

Данные передают через extras: в Intent добавляют пары ключ-значение (строки, числа, логические значения и т.д.), а в целевом Activity извлекают их из intent по тем же ключам. Если нужно передать более сложный объект, его обычно делают Parcelable или, реже, Serializable.

\textbf{13. В каком виде хранятся данные с помощью класса SharedPreferences?}

SharedPreferences сохраняет данные как пары «ключ-значение», которые записываются в XML-файл. Обычно хранят строки, целые числа, long, float, boolean и набор строк (Set<String>). Физически файл располагается в каталоге приложения, например: /data/data/имя\_пакета/shared\_prefs/имя\_prefs.xml.

\textbf{14. В каких случаях целесообразно хранить данные с помощью класса SharedPreferences?}

SharedPreferences удобно применять для небольших и простых данных, которые должны переживать перезапуск приложения: пользовательские настройки, флаги вроде «первый запуск» или «пользователь вошёл», а также небольшие значения профиля. Для больших объёмов, сложных структур и данных, которые часто меняются, лучше использовать базу данных или файлы. Для чувствительной информации предпочтительны защищённые хранилища и/или шифрование.
